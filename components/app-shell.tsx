...
function routePath(pathname: string) {
  const stripped = publicBasePath && pathname.startsWith(publicBasePath)
...
function Shell() {
  ...
  const [path, setPath] = useState(
      routePath(pathname).split('/').filter(Boolean)[0] || '',
    ),
  ...
  useEffect(() => {
    const sync = () => {
      ...
      setPath(routePath(location.pathname).split('/').filter(Boolean)[0] || '');
  ...
  useEffect(() => {
    ...
    const rewrite = () => {
      document
        .querySelectorAll<HTMLAnchorElement>('a[href^="/"], form[action^="/"]')
        .forEach((element) => {
          ...
          if (
            value &&
            value.startsWith('/') &&
            !value.startsWith(publicBasePath + '/') &&
            value !== publicBasePath
          )
            element.setAttribute(attribute, publicBasePath + value);
        });
    };
    rewrite();
